...
import {
  ...
  Plus, RefreshCw, ShieldCheck, TriangleAlert, WalletCards, X,
} from "lucide-react";
      ...
      <div className="admin-grid">
        <section className="panel admin-queue">
          ...
          <div className="admin-filters">
            ...
            <select aria-label="النوع" value={type} onChange={(event) => changeFilter(setType, event.target.value)}><option value="">كل الأنواع</option><option value="expense">سحب</option><option value="payment">إيداع عام</option><option value="loan_payment">إيداع قسط</option></select>
          ...
          <div className="admin-transaction-list">
            ...
            {data?.transactions.transactions.map((item) => <button className="admin-transaction" key={`${item.type}-${item.id}`} onClick={() => setSelected(item)}><span className={`admin-type type-${item.type}`}>{item.type === "expense" ? <WalletCards size={18} /> : <HandCoins size={18} />}</span><div><strong>{item.title}</strong><span>{item.memberName} · {dateFormatter.format(new Date(item.date))}</span></div><div><strong><Money value={item.amount} /></strong><span className={`status-badge status-${item.status}`}>{adminStatus(item.status)}</span></div></button>)}
          </div>
...
function AdminTransactionForm({ data, onSaved }: { data: AdminDashboardData; onSaved: () => void }) {
  ...
  const [type, setType] = useState<"expense" | "payment" | "loan_payment">("expense");
  ...
  async function submit(event: React.FormEvent) {
    ...
    setBusy(true);
    try {
      await jsonRequest("/api/admin/transactions", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ memberId, type, amount: Number(amount), category, installmentId: type === "loan_payment" ? effectiveInstallmentId : undefined, date, note, requestId: crypto.randomUUID() }) });
      setAmount(""); setNote(""); setMessage("تم حفظ العملية واعتمادها."); onSaved();
    } catch (cause) { setMessage(cause instanceof Error ? cause.message : "تعذر الحفظ."); }
    finally { setBusy(false); }
  }

  return <form className="admin-form" onSubmit={submit}><label><span>العضو</span><select value={memberId} onChange={(event) => { setMemberId(Number(event.target.value)); setInstallmentId(undefined); }}>{data.profiles.map((profile) => <option value={profile.id} key={profile.id}>{profile.name}</option>)}</select></label><label><span>النوع</span><select value={type} onChange={(event) => { setType(event.target.value as typeof type); setInstallmentId(undefined); }}><option value="expense">سحب</option><option value="payment">إيداع عام</option><option value="loan_payment">سداد قسط</option></select></label><label><span>المبلغ</span><input required min="0.01" step="0.01" inputMode="decimal" value={amount} onChange={(event) => setAmount(event.target.value)} /></label>{type === "expense" && <label><span>العنوان</span><input required value={category} onChange={(event) => setCategory(event.target.value)} /></label>}{type === "loan_payment" && <label><span>القسط</span><select required value={effectiveInstallmentId ?? ""} onChange={(event) => setInstallmentId(Number(event.target.value))}><option value="" disabled>اختر القسط</option>{installments.map((item) => <option value={item.id} key={item.id}>{item.title} — {moneyFormatter.format(item.remainingAmount)} ريال</option>)}</select></label>}<label><span>التاريخ</span><input required type="date" value={date} onChange={(event) => setDate(event.target.value)} /></label><label className="wide"><span>ملاحظة</span><input value={note} onChange={(event) => setNote(event.target.value)} /></label><div className="wide form-submit-row">{message && <small>{message}</small>}<button className="primary-button" disabled={busy || !amount || (type === "loan_payment" && !effectiveInstallmentId)}><Plus size={18} />{busy ? "جارٍ الحفظ…" : "حفظ واعتماد"}</button></div></form>;
}
...
function AdminDecisionSheet({ transaction, onClose, onSaved }: { transaction: FinancialTransaction; onClose: () => void; onSaved: () => void }) {
  ...
  return <div className="sheet-layer" role="dialog" aria-modal="true" aria-label="قرار العملية"><button className="sheet-backdrop" onClick={onClose} aria-label="إغلاق" /><section className="admin-decision-sheet"><header><div><span>{transaction.memberName}</span><h2>{transaction.title}</h2></div><button onClick={onClose} aria-label="إغلاق"><X size={21} /></button></header><div className="decision-amount"><Money value={transaction.amount} /><span className={`status-badge status-${transaction.status}`}>{adminStatus(transaction.status)}</span></div><div className="decision-details"><span>النوع: {transaction.type}</span><span>التاريخ: {dateFormatter.format(new Date(transaction.date))}</span>{transaction.note && <span>ملاحظة العضو: {transaction.note}</span>}</div>{editable && <><div className="edit-fields"><label><span>المبلغ</span><input type="number" min="0.01" step="0.01" value={amount} onChange={(event) => setAmount(event.target.value)} /></label>{transaction.type === "expense" && <label><span>العنوان</span><input value={title} onChange={(event) => setTitle(event.target.value)} /></label>}<button className="secondary-button" onClick={saveEdit} disabled={busy}><FilePenLine size={17} /> حفظ التعديل</button></div><label className="manager-note"><span>ملاحظة المدير</span><textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder="مطلوبة للتعليق أو الرفض" maxLength={500} /></label>{error && <div className="alert"><TriangleAlert size={16} />{error}</div>}<div className="decision-actions"><button className="approve" onClick={() => void act("approve")} disabled={busy}><Check size={18} /> اعتماد</button><button className="hold" onClick={() => void act("hold")} disabled={busy}><CirclePause size={18} /> تعليق</button><button className="reject" onClick={() => void act("reject")} disabled={busy}><X size={18} /> رفض</button></div></>}</section></div>;
}

function adminStatus(status: FinancialTransaction["status"]): string { return status === "approved" ? "معتمد" : status === "rejected" ? "مرفوض" : status === "on_hold" ? "معلّقة" : "بانتظار"; }
