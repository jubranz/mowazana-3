...
function AdminDecisionSheet({ transaction, onClose, onSaved }: { transaction: FinancialTransaction; onClose: () => void; onSaved: () => void }) {
  ...
  return <div className="sheet-layer" role="dialog" aria-modal="true" aria-label="قرار العملية"><button className="sheet-backdrop" onClick={onClose} aria-label="إغلاق" /><section className="admin-decision-sheet"><header><div><span>{transaction.memberName}</span><h2>{transaction.title}</h2></div><button onClick={onClose} aria-label="إغلاق"><X size={21} /></button></header><div className="decision-amount"><Money value={transaction.amount} /><span className={`status-badge status-${transaction.status}`}>{adminStatus(transaction.status)}</span></div><div className="decision-details"><span>النوع: {transaction.type}</span><span>التاريخ: {dateFormatter.format(new Date(transaction.date))}</span>{transaction.note && <span>الوصف: {transaction.note}</span>}{transaction.imageUrl && <a href={transaction.imageUrl} target="_blank" rel="noreferrer">عرض الصورة المرفقة</a>}</div>{editable && <><div className="edit-fields"><label><span>المبلغ</span><input type="number" min="0.01" step="0.01" value={amount} onChange={(event) => setAmount(event.target.value)} /></label>{transaction.type === "expense" && <label><span>العنوان</span><input value={title} onChange={(event) => setTitle(event.target.value)} /></label>}<button className="secondary-button" onClick={saveEdit} disabled={busy}><FilePenLine size={17} /> حفظ التعديل</button></div><label className="manager-note"><span>ملاحظة المدير</span><textarea value={note} onChange={(event) => setNote(event.target.value)} placeholder="مطلوبة للتعليق أو الرفض" maxLength={500} /></label>{error && <div className="alert"><TriangleAlert size={16} />{error}</div>}<div className="decision-actions"><button className="approve" onClick={() => void act("approve")} disabled={busy}><Check size={18} /> اعتماد</button><button className="hold" onClick={() => void act("hold")} disabled={busy}><CirclePause size={18} /> تعليق</button><button className="reject" onClick={() => void act("reject")} disabled={busy}><X size={18} /> رفض</button></div></>}</section></div>;
}
